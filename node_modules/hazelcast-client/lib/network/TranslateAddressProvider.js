...
class TranslateAddressProvider {
    ...
    internalMemberAddressMatchesConfig(members) {
        const pHostsFromConfig = this.config.network.clusterMembers
            .map((address) => {
            return AddressUtil_1.resolveAddress(address).catch(() => null);
        });
        ...
        return Promise.all(pHostsFromConfig)
            ...
            const memberHostPromises = members
                .map((member) => {
                return AddressUtil_1.resolveAddress(member.address.host).catch(() => null);
            });
    ...
    reachableOnlyViaPublicAddress(members) {
        ...
        Util_1.shuffleArray(shuffledMembers);
    ...
    reachableOnlyViaPublicAddressInternal(shuffledMembers, attempt) {
        ...
        return Promise.all([
            AddressUtil_1.isAddressReachable(internalAddress.host, internalAddress.port, this.internalAddressTimeoutMs),
            AddressUtil_1.isAddressReachable(publicAddress.host, publicAddress.port, this.publicAddressTimeoutMs)
